...
import {AccessControl} from "./libraries/openzeppelin/AccessControl.sol";
import {TimelockController} from "./libraries/openzeppelin/TimelockController.sol";
import {Address} from "./libraries/openzeppelin/Address.sol";
...
contract Chief is CoreRoles, AccessControl, IChief {
   ...
   */
  function pauseForceVaults(IVaultPausable[] calldata vaults) external onlyRole(PAUSER_ROLE) {
    bytes memory data = abi.encodeWithSelector(IVaultPausable.pauseForceAll.selector);
   ...
   */
  function unpauseForceVaults(IVaultPausable[] calldata vaults) external onlyRole(UNPAUSER_ROLE) {
    bytes memory callData = abi.encodeWithSelector(IVaultPausable.unpauseForceAll.selector);
  ...
  function pauseActionInVaults(
    IVaultPausable[] calldata vaults,
    VaultActions action
  )
    external
    onlyRole(PAUSER_ROLE)
  {
    bytes memory callData = abi.encodeWithSelector(IVaultPausable.pause.selector, action);
    _changePauseState(vaults, callData);
  }
  ...
  {
    bytes memory callData = abi.encodeWithSelector(IVaultPausable.unpause.selector, action);
    _changePauseState(vaults, callData);
  }
   ...
   */
  function _changePauseState(IVaultPausable[] memory vaults, bytes memory data) internal {
  ...
  function _setVaultStatus(address vault, bool active) internal {
    ...
    if (active == false) {
      ...
      IVaultPausable(vault).pause(VaultActions.Deposit);

      // If `vault` is a {BorrowingVault}.
      if (IVault(vault).debtAsset() != address(0)) {
        IVaultPausable(vault).pause(VaultActions.Borrow);
      }
    }
